...
private:
  typedef T value_t;
  typedef HeapWithLock<T, Comparer, 4, Prior> Heap;
...
//      }
      ...
      if (/** todo for adap local_q < getNQ() &&*/ change >= PopChange::P) {
        heap_i = get_heap_ptr(local_q);
        if (try_lock_heap(local_q)) {
...
//          local_q = nQ;
          ...
          local->min.store(!local->heap.empty() ? local->heap.min().prior() : local->usedT.prior(), std::memory_order_release);
