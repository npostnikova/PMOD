...
def generate_amqs(graph_type,
    ...
    script = open(f"/home/ubuntu/PMOD/heatmaps/heatmap_run_fake.sh", "w")
    smq_vals = [(8, 1), (2, 16), (32, 4), (8, 16), (16, 8), (1024, 1024), (4, 8), (512, 1)]
    amq_vals = [(1024, 4), (1024, 256), (1024, 1024), (1024, 512)]
    for (p, q) in smq_vals:
        wl = "smq"
        suff = f"1_{p}_{q}"
        amq_params = [
            ...
            f"Prob<1, {p}>",  # Steal probability
            "true",  # Concurrent
            str(q)  # Steal batch size
        ]
        header.write(
    ...
    for (p, q) in amq_vals:
        ...
        amq_params = [
            ...
            "true",  # Concurrent
            "false",  # Blocking
            f"Prob<1, {p}>",  # Push probability
            f"Prob<1, {q}>",  # Pop probability
...
if __name__ == "__main__":
    ...
    parser.add_argument('--runs', type=int,
                        help='Number of runs', default=0)
    # parser.add_argument('--wl',
    #                     help='Supported wls: amq, smq')
    args = parser.parse_args()
    generate_amqs(args.graph, args.algo, args.runs)
